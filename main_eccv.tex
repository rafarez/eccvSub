...
%%%%%%%%% ABSTRACT
\begin{abstract}
   In this paper, we propose an improvement to the Exemplar SVM (ESVM) feature encoding pipeline first proposed by Zepeda and P\'erez \cite{ZePe15}.  We first show that, by replacing the hinge loss by the square-loss in the ESVM cost function, we obtain similar results on image retrieval in a fraction of the execution time. We call this method Square-Loss Exemplar Machine, or SLEM. Secondly, we introduce a efficient kernel implementation for SLEM. This implementation scales well for image
\end{abstract}
